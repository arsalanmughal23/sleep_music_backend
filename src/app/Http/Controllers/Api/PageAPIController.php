...
class PageAPIController extends AppBaseController
{
     ...
     *      @SWG\Parameter(
     *          name="slug",
     *          description="Get all pages that matches this slug",
     *          type="string",
     *          required=false,
     *          in="query"
     *      ),
    ...
    public function index(Request $request)
    {
        $pages = $this->pageRepository
            ...
            ->pushCriteria(new PageCriteria($request->only(['slug'])))
            ->where('status', 1)
            ->get();
    ...
    {
        $page = $this->pageRepository->pushCriteria(new PageCriteria(['slug' => $slug]))->first();
        if (empty($page)) {
